...
const QUESTIONS = [
  {
    ...
    question: "Explain the concept of 'Closure' in JavaScript with a real-world analogy.",
  ...
  },
  {
    id: 8, type: "viva",
    section: "Section 3", sectionFull: "Viva & Reasoning",
    question: "Describe the difference between Relational and Non-Relational databases.",
    placeholder: "Type your explanation here...",
    duration: 60,
  },
  {
  ...
  },
  {
    id: 10, type: "viva",
    section: "Section 3", sectionFull: "Viva & Reasoning",
    question: "Explain the difference between undefined and null.",
    placeholder: "Type your explanation here...",
    duration: 60,
  }
];
  ...
  if (recorder) {
    ...
    recorder.onstop = async () => {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_VOICE_API}/transcribe`, {
          method: "POST",
          body: formData,
        });
...
const sendAudio = async () => {
  ...
  const mcqScore = () =>
    QUESTIONS.reduce((acc, q, i) =>
      q?.question_type === "mcq" && answers[i] === q.correct ? acc + 1 : acc, 0);
  ...
  const goNext = useCallback((auto = false) => {
    ...
    setTimeout(() => {
      if (qNumber < 10) {
  // nothing needed (handled by submitAnswerHandler)
} else {
  doSubmit();
}
      setTransitioning(false);
    }, 180);
  }, [transitioning, qNumber]);
  ...
  const progressPct = (qNumber / 10) * 100;
        ...
        <header className="ex-bar">
          ...
          <div className="ex-bar__center">
            <div className="ex-bar__prog-wrap">
              ...
              <span className="ex-bar__prog-label">Question {qNumber}</span>
            </div>
          </div>
  ...
  const isCur = i === (qNumber - 1);
  const isDone = i < (qNumber - 1);
      ...
      {isDone ? "✓" : i + 1}
            ...
            <div className="ex-sidebar__block">
              ...
              {SECTIONS_META.map(sec => {
                const done =
  question?.question_type === sec.type
    ? 1
    : 0;
          ...
          <main className={`ex-main ${transitioning ? "ex-main--fade" : ""}`}>
            ...
            <div className="ex-q-card">
              <div className="ex-q-num" style={{ color: secColor }}>Q{qNumber}</div>
...
>
                {qNumber < 10 ? "Next Question →" : "Submit Exam ✓"}
</button>
            </div>
